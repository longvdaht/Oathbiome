...
(function () {
    ...
    */
    function elementsIntersectionObserver() {
      const elementsToAnimate = document.querySelectorAll(selectors$w.aosIndividual);
    ...
    class Accessibility {
      ...
      focusEvents() {
        document.addEventListener('mousedown', () => {
          this.body.classList.remove(classes$n.focus);
        });
        ...
        document.addEventListener('keyup', (event) => {
          ...
          this.body.classList.add(classes$n.focus);
        });
      }
    }
    ...
    */
    ...
    class CartItems extends HTMLElement {
      ...
      connectedCallback() {
        ...
        this.checkoutButtons = document.querySelector(selectors$t.checkoutButtons);
      ...
      onCartDrawerClose() {
        ...
        if (this.cartDrawer?.classList.contains(classes$m.open)) {
          this.cart.classList.remove(classes$m.updated);
        }

        this.cartEmpty.classList.remove(classes$m.updated);
        this.cartErrorHolder.classList.remove(classes$m.expanded);
        this.cart.querySelectorAll(selectors$t.animation).forEach((item) => {
          const removeHidingClass = () => {
            item.classList.remove(classes$m.hiding);
            item.removeEventListener('animationend', removeHidingClass);
          };

          item.classList.add(classes$m.hiding);
      ...
      cartRemoveEvents() {
        ...
        cartItemRemove.forEach((button) => {
          ...
          button.addEventListener('click', (event) => {
            ...
            if (button.classList.contains(classes$m.disabled)) return;
      ...
      cartAddEvent(event) {
        ...
        const hasInputsInNoScript = [...form.elements].some((el) => el.closest(selectors$t.noscript));
      ...
      handleFormDataDuplicates(elements, formData) {
        ...
        elements.forEach((element) => {
          if (element.closest(selectors$t.noscript)) {
      ...
      formSubmitHandler() {
        ...
        if (!termsAccepted) {
          ...
          termsError.classList.add(classes$m.expanded);
        } else {
          termsError.classList.remove(classes$m.expanded);
      ...
      formErrorsEvents(errorContainer) {
        ...
        buttonErrorClose?.addEventListener('click', (e) => {
          ...
          if (errorContainer) {
            errorContainer.classList.remove(classes$m.visible);
          }
        });
      }
      ...
      addToCart(formData, button) {
        if (this.cart) {
          this.cart.classList.add(classes$m.loading);
        }

        const quickAddHolder = button?.closest(selectors$t.quickAddHolder);

        if (button) {
          button.classList.add(classes$m.loading);
          button.disabled = true;
        }

        if (quickAddHolder) {
          quickAddHolder.classList.add(classes$m.visible);
        }
        ...
        fetch(theme.routes.cart_add_url, {
          ...
          .then((response) => {
            if (response.status) {
              ...
              if (button) {
                button.classList.remove(classes$m.loading);
            ...
            if (this.cart) {
              if (button) {
                button.classList.remove(classes$m.loading);
                button.classList.add(classes$m.added);
      ...
      updateCart(updateData = {}, currentItem = null) {
        ...
        if (currentItem !== null) {
          if (updatedQuantity) {
            currentItem.classList.add(classes$m.loading);
          } else {
            currentItem.classList.add(classes$m.removed);
          }
        }
      ...
      resetLineItem(item) {
        ...
        qtyInput.value = qty;
        item.classList.remove(classes$m.loading);
      }
      ...
      disableCartButtons() {
        ...
        if (inputs.length) {
          inputs.forEach((item) => {
            item.classList.add(classes$m.disabled);
            item.blur();
            item.disabled = true;
          });
        }
      ...
      enableCartButtons() {
        ...
        if (inputs.length) {
          inputs.forEach((item) => {
            item.classList.remove(classes$m.disabled);
            item.disabled = false;
          });
        }
        ...
        this.cart.classList.remove(classes$m.loading);
      }
      ...
      toggleErrorMessage() {
        ...
        this.cartErrorHolder.classList.toggle(classes$m.expanded, this.cartUpdateFailed);
      ...
      addToCartError(data, button) {
        if (button !== null) {
          ...
          if (errorContainer) {
            ...
            errorContainer.classList.add(classes$m.visible);
      ...
      productAddCallback(event) {
        ...
        setTimeout(() => {
          buttons.forEach((button) => {
            button.classList.remove(classes$m.added);
            const isVariantUnavailable =
              button.closest(selectors$t.formWrapper)?.classList.contains(classes$m.variantSoldOut) || button.closest(selectors$t.formWrapper)?.classList.contains(classes$m.variantUnavailable);
          ...
          quickAddHolder?.classList.remove(classes$m.visible);
        }, 1000);
      }
      ...
      toggleClassesOnContainers() {
        ...
        this.cart.classList.toggle(classes$m.empty, !hasItemsInCart);
      ...
      build(data) {
        ...
        if (this.cartDrawer?.classList.contains(classes$m.open)) {
          this.cart.classList.add(classes$m.updated);
        }
        ...
        if (!this.hasItemsInCart()) {
          this.cartEmpty.querySelectorAll(selectors$t.animation).forEach((item) => {
            item.classList.remove(classes$m.animated);
          });
        }
      ...
      freeShippingMessageHandle(total) {
        ...
        this.freeShipping.forEach((message) => {
          ...
          message.classList.toggle(classes$m.success, hasQualifiedShippingMessage && total >= this.freeShippingLimit);
        });
      }
      ...
      animateItems(e) {
        requestAnimationFrame(() => {
          ...
          cart?.querySelectorAll(selectors$t.animation).forEach((item) => {
            item.classList.add(classes$m.animated);
          });
        });
      }
    }
    ...
    class CartDrawer extends HTMLElement {
      constructor() {
        ...
        this.cartDrawerClose = this.querySelector(selectors$s.cartDrawerClose);
        this.cartDrawerInner = this.querySelector(selectors$s.cartDrawerInner);
        ...
        this.cartDrawerSection = this.closest(selectors$s.shopifySection);
        this.a11y = window.theme.a11y;
      ...
      connectedCallback() {
        const drawerSection = this.closest(selectors$s.shopifySection);
        ...
        if (window.theme.hasCartDrawer) {
          if (!window.Shopify.designMode) {
            ...
            errorMessage.classList.add(classes$l.drawerEditorError);
            errorMessage.innerText = 'Cart drawer section already exists.';

            if (!this.querySelector(`.${classes$l.drawerEditorError}`)) {
              this.querySelector(selectors$s.cartDrawerInner).append(errorMessage);
            }

            this.classList.add(classes$l.duplicate);
          }
        }
      ...
      disconnectedCallback() {
        ...
        if (document.querySelectorAll(selectors$s.cartDrawer).length <= 1) {
      ...
      openCartDrawerOnSelect(e) {
        const cartDrawerSection = e.target.querySelector(selectors$s.shopifySection) || e.target.closest(selectors$s.shopifySection) || e.target;
      ...
      openCartDrawer(forceOpen = false) {
        if (!forceOpen && this.classList.contains(classes$l.duplicate)) return;
        ...
        this.classList.add(classes$l.open);
        ...
        window.theme.waitForAnimationEnd(this.cartDrawerInner).then(() => {
          this.a11y.trapFocus(this, {
            elementToFocus: this.querySelector(selectors$s.cartDrawerClose),
          });
        });
      }
      ...
      closeCartDrawer() {
        if (!this.classList.contains(classes$l.open)) return;

        this.classList.add(classes$l.closing);
        this.classList.remove(classes$l.open);
        ...
        window.theme.waitForAnimationEnd(this.cartDrawerInner).then(() => {
          this.classList.remove(classes$l.closing);
        });
      }
      ...
      observeAdditionalCheckoutButtons() {
        ...
        const additionalCheckoutButtons = this.querySelector(selectors$s.additionalCheckoutButtons);
      ...
      connectedCallback() {
        ...
        this.collapsibles.forEach((collapsible) => {
          ...
          body?.addEventListener('transitionend', (event) => {
            ...
            if (collapsible.getAttribute(attributes$g.open) == 'false') {
      ...
      toggle() {
        ...
        this.collapsibles.forEach((collapsible) => {
          ...
          if (isEligible) {
            ...
            collapsible.querySelector(selectors$r.trigger).removeAttribute('tabindex');
            collapsible.removeAttribute(attributes$g.open);
            ...
            collapsible.querySelector(selectors$r.trigger).setAttribute('tabindex', -1);
          }
        });
      }
    ...
    class DeferredMedia extends HTMLElement {
      constructor() {
        ...
        const poster = this.querySelector(selectors$q.deferredMediaButton);
      ...
      loadContent(focus = true) {
        ...
        if (!this.getAttribute(attributes$f.loaded)) {
          ...
          const templateContent = this.querySelector(selectors$q.template).content.firstElementChild.cloneNode(true);
          content.appendChild(templateContent);
          this.setAttribute(attributes$f.loaded, true);

          const mediaElement = this.appendChild(content.querySelector(selectors$q.media));
      ...
      pauseAllMedia() {
        document.querySelectorAll(selectors$q.youtube).forEach((video) => {
          video.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
        });
        document.querySelectorAll(selectors$q.vimeo).forEach((video) => {
          video.contentWindow.postMessage('{"method":"pause"}', '*');
        });
        document.querySelectorAll(selectors$q.video).forEach((video) => video.pause());
        document.querySelectorAll(selectors$q.productModel).forEach((model) => {
    ...
    */
    ...
    class DraggableSlider {
      ...
      scrollToSlide() {
        ...
        const slide = this.slider.querySelector(`${selectors$p.slide}.${classes$j.visible}`);
        if (!slide) return;
    ...
    if (!customElements.get('grid-slider')) {
      customElements.define(
        ...
        class GridSlider extends HTMLElement {
          ...
          init() {
            this.slider = this.querySelector(selectors$o.slider);
            this.slides = this.querySelectorAll(selectors$o.slide);
            this.buttons = this.querySelectorAll(selectors$o.buttonArrow);
            this.slider.classList.add(classes$i.scrollSnapDisabled);
            this.toggleSlider();
            ...
            window.theme
              ...
              .then(() => {
                this.slider.classList.remove(classes$i.scrollSnapDisabled);
              })
              .catch(() => {
                this.slider.classList.remove(classes$i.scrollSnapDisabled);
              });
          ...
          toggleSlider() {
            ...
            if (isEnabled && (!window.theme.isMobile() || !window.theme.touch)) {
              ...
              this.slidesObserver = new IsInView(this.slider, selectors$o.slide);
          ...
          buttonArrowClickEvent(e) {
            ...
            const firstVisibleSlide = this.slider.querySelector(`${selectors$o.slide}.${classes$i.visible}`);
            let slide = null;
          ...
          getSlidesWidth() {
            return this.slider.querySelector(selectors$o.slide)?.clientWidth * this.slider.querySelectorAll(selectors$o.slide).length;
          }
          ...
          positionArrows() {
            const targetElement =
              this.slider.querySelector(selectors$o.productImage) || this.slider.querySelector(selectors$o.collectionImage) || this.slider.querySelector(selectors$o.columnImage) || this.slider;
    ...
    */
      ...
      customElements.define(
        ...
        class HeaderComponent extends HTMLElement {
          constructor() {
            ...
            this.desktop = this.querySelector(selectors$n.desktop);
            ...
            document.body.classList.toggle(classes$h.sticky, this.isSticky);

            // Fallback for CSS :has() selectors
            let enableTransparentHeader = false;
            const firstSectionOverlayHeader = document.querySelector(selectors$n.firstSectionOverlayHeader);
            if (firstSectionOverlayHeader && !firstSectionOverlayHeader.querySelector(selectors$n.preventTransparent)) {
              enableTransparentHeader = true;
            }

            document.body.classList.toggle(classes$h.transparent, this.hasAttribute(attributes$d.transparent));
            document.body.classList.toggle(classes$h.firstSectionOverlayHeader, enableTransparentHeader);
          }
          ...
          checkWidth() {
            if (document.body.clientWidth < this.minWidth) {
              this.classList.add(classes$h.showMobileClass);

              // Update --header-height CSS variable when switching to a mobile nav
              const {headerHeight} = window.theme.readHeights();
              document.documentElement.style.setProperty('--header-height', `${headerHeight}px`);
            } else {
              this.classList.remove(classes$h.showMobileClass);
            }
          }

          getMinWidth() {
            const comparitor = document.createElement('div');
            comparitor.classList.add(classes$h.clone, classes$h.headerWrapper);
            comparitor.appendChild(this.querySelector('header').cloneNode(true));
            document.body.appendChild(comparitor);
            const widthWrappers = comparitor.querySelectorAll(selectors$n.widthContentWrapper);
            let minWidth = 0;
            let spaced = 0;

            widthWrappers.forEach((context) => {
              const wideElements = context.querySelectorAll(selectors$n.widthContent);
              let thisWidth = 0;
          ...
          cartToggleEvent() {
            ...
            this.querySelectorAll(selectors$n.cartToggleButton)?.forEach((button) => {
              button.addEventListener('click', (e) => {
                const cartDrawer = document.querySelector(selectors$n.cartDrawer);
          ...
          initSticky() {
            ...
            this.headerOffset = document.querySelector(selectors$n.pageHeader)?.offsetTop;
      ...
      customElements.define(
        ...
        class HoverDisclosure extends HTMLElement {
          constructor() {
            ...
            this.wrapper = this.closest(selectors$m.wrapper);
            this.key = this.getAttribute('aria-controls');
            this.link = this.querySelector(selectors$m.link);
          ...
          showDisclosure(e) {
            ...
            this.classList.add(classes$g.isVisible);
          ...
          hideDisclosure() {
            this.classList.remove(classes$g.isVisible);
          ...
          staggerChildAnimations() {
            const simple = this.querySelectorAll(selectors$m.stagger);
            let step = 50;
            ...
            const pairs = this.querySelectorAll(selectors$m.staggerPair);
          ...
          handleTablets() {
            ...
            this.addEventListener(
              'touchstart',
              function (e) {
                const isOpen = this.classList.contains(classes$g.isVisible);
                if (!isOpen) {
                  e.preventDefault();
        ...
        class HeaderDrawer extends HTMLElement {
          ...
          connectDrawer() {
            this.addEventListener('theme:drawer:toggle', (e) => {
              ...
              if (this.classList.contains(classes$f.open)) {
                this.dispatchEvent(
          ...
          showDrawer() {
            ...
            this.classList.add(classes$f.open, classes$f.animated);
          ...
          hideDrawer() {
            if (this.isAnimating || !this.classList.contains(classes$f.open)) return;
            ...
            if (this.triggerButton) {
              ...
              if (document.body.classList.contains(classes$f.isFocused)) {
            ...
            window.theme.waitForAnimationEnd(this.drawerInner).then(() => {
              this.classList.remove(classes$f.closing, classes$f.animated);
      ...
      customElements.define(
        ...
        class HeaderMobileSliderule extends HTMLElement {
          constructor() {
            ...
            this.key = this.id;
            this.sliderule = this.querySelector(selectors$k.sliderule);
            const btnSelector = `[${selectors$k.slideruleOpen}='${this.key}']`;
            this.exitSelector = `[${selectors$k.slideruleClose}='${this.key}']`;
            this.trigger = this.querySelector(btnSelector);
            this.exit = document.querySelectorAll(this.exitSelector);
            this.pane = this.trigger.closest(`[${selectors$k.sliderulePane}]`);
            this.childrenElements = this.querySelectorAll(selectors$k.children);
            this.drawerContent = this.closest(selectors$k.drawerContent);
          ...
          trapFocusSliderule(showSliderule = true) {
            ...
            if (trapFocusButton && this.drawerContent) {
              this.a11y.trapFocus(this.drawerContent, {
                elementToFocus: document.body.classList.contains(classes$e.focused) ? trapFocusButton : null,
              });
            }
          }
          ...
          hideSliderule(close = false) {
            ...
            this.pane.setAttribute(selectors$k.sliderulePane, newPosition);
            this.pane.classList.add(classes$e.isHiding);
            this.sliderule.classList.add(classes$e.isHiding);
            const hiddenSelector = close ? `[${selectors$k.animates}].${classes$e.isHidden}` : `[${selectors$k.animates}="${newPosition}"]`;
            ...
            const children = close ? this.pane.querySelectorAll(`.${classes$e.isVisible}, .${classes$e.isHiding}`) : this.childrenElements;
            children.forEach((element, index) => {
              const lastElement = children.length - 1 == index;
              element.classList.remove(classes$e.isVisible);
              if (close) {
                element.classList.remove(classes$e.isHiding);
                this.pane.classList.remove(classes$e.isHiding);
              }
              const removeHidingClass = () => {
                if (parseInt(this.pane.getAttribute(selectors$k.sliderulePane)) === newPosition) {
          ...
          showSliderule() {
            ...
            const parent = this.closest(`.${classes$e.isVisible}`);
            ...
            lastScrollableElement.classList.add(classes$e.scrolling);

            const lastScrollableIsScrolling = () => {
              if (lastScrollableElement.scrollTop <= 0) {
                lastScrollableElement.classList.remove(classes$e.scrolling);
            ...
            const hiddenItems = this.pane.querySelectorAll(`[${selectors$k.animates}="${oldPosition}"]`);
            if (hiddenItems.length) {
              hiddenItems.forEach((element, index) => {
                const lastElement = hiddenItems.length - 1 == index;
                element.classList.add(classes$e.isHiding);
                const removeHidingClass = () => {
                  element.classList.remove(classes$e.isHiding);
                  if (parseInt(this.pane.getAttribute(selectors$k.sliderulePane)) !== oldPosition) {
          ...
          closeSliderule() {
            if (this.pane && this.pane.hasAttribute(selectors$k.sliderulePane) && parseInt(this.pane.getAttribute(selectors$k.sliderulePane)) > 0) {
              this.hideSliderule(true);
              if (parseInt(this.pane.getAttribute(selectors$k.sliderulePane)) > 0) {
                this.pane.setAttribute(selectors$k.sliderulePane, 0);
              }
            }
          }
    ...
    class SearchPopdown extends HTMLElement {
      constructor() {
        super();
        this.popdown = this.querySelector(selectors$j.popdown);
        this.popdownContainer = this.querySelector(selectors$j.details);
        this.popdownClose = this.querySelector(selectors$j.popdownClose);
        ...
        this.mobileMenu = this.closest(selectors$j.mobileMenu);
        this.a11y = window.theme.a11y;
      }
      ...
      popdownTransitionCallback(event) {
        ...
        if (!this.classList.contains(classes$d.open)) {
          ...
          this.a11y.trapFocus(this.popdown, {
            elementToFocus: this.popdown.querySelector(selectors$j.input),
          });
        }
      }
      ...
      customElements.define(
        ...
        class NativeScrollbar extends HTMLElement {
          constructor() {
            ...
            this.scrollbar = this.querySelector(selectors$g.scrollbar);
            this.arrowNext = this.querySelector(selectors$g.scrollbarArrowNext);
            this.arrowPrev = this.querySelector(selectors$g.scrollbarArrowPrev);
    ...
    if (!customElements.get('popout-select')) {
      customElements.define(
        ...
        class Popout extends HTMLElement {
          ...
          connectedCallback() {
            this.popoutList = this.querySelector(selectors$f.popoutList);
            this.popoutToggle = this.querySelector(selectors$f.popoutToggle);
            this.popoutToggleText = this.querySelector(selectors$f.popoutToggleText);
            this.popoutInput = this.querySelector(selectors$f.popoutInput) || this.parentNode.querySelector(selectors$f.popoutInput);
            this.popoutOptions = this.querySelectorAll(selectors$f.popoutOptions);
          ...
          toggleListPosition() {
            const button = this.querySelector(selectors$f.popoutToggle);
            ...
            const removeTopClass = () => {
              if (button.getAttribute(attributes$a.ariaExpanded) !== 'true') {
                this.popoutList.classList.remove(classes$a.popoutListTop);
              }
            ...
            if (button.getAttribute(attributes$a.ariaExpanded) === 'true') {
              if (window.innerHeight / 2 < popoutTop) {
                this.popoutList.classList.add(classes$a.popoutListTop);
              }
            } else {
          ...
          popupOptionsClick(evt) {
            const link = evt.target.closest(selectors$f.popoutOptions);
            ...
            if (link.attributes.href.value === '#') {
              ...
              if (this.fireSubmitEvent) {
                ...
                const listTargetElement = this.popoutList.querySelector(`.${classes$a.active}`);
                const targetAttribute = this.popoutList.querySelector(`[${attributes$a.ariaCurrent}]`);

                this.popoutInput.dispatchEvent(new Event('change'));

                if (listTargetElement) {
                  listTargetElement.classList.remove(classes$a.active);
                  currentTarget.classList.add(classes$a.active);
                }

                if (this.popoutInput.name == 'quantity' && !currentTarget.nextSibling) {
                  this.classList.add(classes$a.active);
                }
    ...
    class PopupComponent extends HTMLElement {
      constructor() {
        ...
        this.popup = this.querySelector(selectors$e.dialog);
        ...
        this.buttonPopupOpen = this.querySelector(selectors$e.open);
        this.a11y = window.theme.a11y;
        this.isAnimating = false;
      ...
      bindListeners() {
        ...
        this.popup.querySelectorAll(selectors$e.close)?.forEach((closeButton) => {
      ...
      customElements.define(
        ...
        class PredictiveSearch extends HeaderSearchForm {
          constructor() {
            ...
            this.cachedResults = {};
            this.input = this.querySelector(selectors$d.searchInput);
            this.isOpen = false;
            this.predictiveSearchResults = this.querySelector(selectors$d.predictiveSearchResults);
            this.searchPopdown = this.closest(selectors$d.searchPopdown);
            this.popularSearches = this.searchPopdown?.querySelector(selectors$d.popularSearches);
            this.searchTerm = '';
          }
          ...
          getSearchResults(searchTerm) {
            ...
            fetch(`${theme.routes.predictive_search_url}?q=${encodeURIComponent(searchTerm)}&section_id=predictive-search`, {signal: this.abortController.signal})
              ...
              .then((text) => {
                const resultsMarkup = new DOMParser().parseFromString(text, 'text/html').querySelector(selectors$d.sectionPredictiveSearch).innerHTML;
    ...
    class ProductFormReader {
      ...
      plan(variant) {
        ...
        const sellingPlanChecked = this.element.querySelector(`${selectors$c.planInput}:checked`);
      ...
      selectedOptionValues() {
        const selectedOptionValues = Array.from(this.element.querySelectorAll(`${selectors$c.optionInput}`)).map((input) => input.value);
        return {
    ...
    class SelloutVariants {
      ...
      update() {
        ...
        this.optionElements.forEach((el) => {
          const parent = el.closest(`[${selectors$b.optionPosition}]`);
          if (!parent) return;
          const val = el.value || el.getAttribute(selectors$b.selectOptionValue);
          const positionString = parent.getAttribute(selectors$b.optionPosition);
          ...
          const selectPopout = el.closest(selectors$b.popout);
      ...
      getCurrentState() {
        ...
        const options = this.container.querySelectorAll(selectors$b.optionInputCurrent);
        if (options.length) {
    ...
    class ProductForm extends HTMLElement {
      ...
      connectedCallback() {
        ...
        this.container = this.closest(selectors$a.section) || this.closest(selectors$a.quickAddModal);
        ...
        this.productNotification = this.container.querySelector(selectors$a.productNotification);
        this.productImages = this.container.querySelector(selectors$a.productImages);
        this.productMediaList = this.container.querySelector(selectors$a.productMediaList);
        this.installmentForm = this.container.querySelector(selectors$a.installment);
        this.skuWrapper = this.container.querySelector(selectors$a.variantSku);
        this.sellout = null;
        ...
        this.planDescription = this.container.querySelector(selectors$a.subDescription);
        ...
        this.isPreOrder = this.container.querySelector(selectors$a.isPreOrder);
        ...
        this.variantOptionImages = this.container.querySelectorAll(selectors$a.variantOptionImage);
        this.variantButtons = this.container.querySelectorAll(selectors$a.variantButtons);
      ...
      cartAddEvents() {
        this.buttonATC = this.querySelector(selectors$a.addToCart);
        ...
        this.buttonATC.addEventListener('click', (e) => {
          ...
          if (!this.closest(selectors$a.quickAddModal)) {
      ...
      updateAddToCartState(formState) {
        ...
        if (this.installmentForm && variant) {
          const installmentInput = this.installmentForm.querySelector(selectors$a.inputId);
        ...
        if (formWrapper.length) {
          formWrapper.forEach((element) => {
            if (variant) {
              ...
              const inputId = element.querySelector(`${selectors$a.inputId}[form]`);
              if (inputId) {
      ...
      updateNotificationForm(formState) {
        ...
        const variantTitle = this.productNotification.querySelector(selectors$a.variantTitle);
        const notificationProduct = this.productNotification.querySelector(selectors$a.notificationProduct);
      ...
      isVariantFinalSale(variant) {
        const metafieldsData = document.querySelector(selectors$a.variantFinalSaleMeta)?.textContent;
      ...
      updateSaleTextStandard(formState) {
        const variant = formState.variant;
        const finalSaleBadge = this.priceOffWrap?.querySelector(selectors$a.finalSaleBadge);
        const priceOffBadge = this.priceOffWrap?.querySelector(selectors$a.priceOffBadge);
      ...
      updateProductUnits(formState) {
        ...
        if (unitPrice) {
          ...
          this.container.querySelector(selectors$a.unitWrapper).classList.remove(classes$8.hidden);
        } else {
          this.container.querySelector(selectors$a.unitWrapper).classList.add(classes$8.hidden);
        }
      }
      ...
      updateLegend(formState) {
        const variant = formState.variant;
        if (variant) {
          const optionValues = this.container.querySelectorAll(selectors$a.optionValue);
          if (optionValues.length) {
            optionValues.forEach((optionValue) => {
              const selectorWrapper = optionValue.closest(selectors$a.optionPosition);
              if (selectorWrapper) {
      ...
      init() {
        ...
        this.swatches.forEach((swatch) => {
          ...
          if (variant) {
            ...
            const swatchButton = swatchTemplate.querySelector(selectors$9.swatchButton);
            const swatchLink = swatchTemplate.querySelector(selectors$9.swatchLink);
            const swatchText = swatchTemplate.querySelector(selectors$9.swatchText);
        ...
        this.swatchElements = this.querySelectorAll(selectors$9.swatchLink);
        this.swatchForm = this.productInfo.querySelector(selectors$9.gridSwatchForm);
        ...
        if (this.swatchCount.hasAttribute(attributes$7.swatchCount)) {
          if (this.swatchesStyle == 'text' || this.swatchesStyle == 'text-slider') {
            ...
            this.swatchCount.addEventListener('mouseenter', () => {
              ...
              this.swatchForm.classList.add(classes$7.visible);
            });
            ...
            this.productInfo.addEventListener('mouseleave', () => {
              this.hideSwatchesTimer = setTimeout(() => {
                ...
                this.swatchForm.classList.remove(classes$7.visible);
              }, 100);
            });
          }
    ...
    class HoverImages extends HTMLElement {
      ...
      initFlickity() {
        if (this.querySelectorAll(selectors$8.slide).length < 2) return;
        ...
        this.closest(selectors$8.productLink).addEventListener('click', (e) => {
    ...
    class QuickAddProduct extends HTMLElement {
      ...
      modalCreate(response) {
        const cachedModal = document.querySelector(`${selectors$7.quickAddModal}[${attributes$6.productId}="${this.productId}"]`);
      ...
      errorHandler() {
        this.quickAddHolder.addEventListener('theme:cart:error', (event) => {
          ...
          parentProduct.querySelector(selectors$7.productInformationHolder);
          ...
          initProductSwapUtility() {
            this.postProcessHtmlCallbacks.push((newNode) => {
              if (this.pendingSwapHandle) {
                const swapHandle = newNode.querySelector(`[${attributes$4.swapHandle}="${this.pendingSwapHandle}"]`);
                swapHandle?.focus();
